use crate::errors;
use crate::instructions::LessonStarted;
...
pub struct LessonApprove<'info> {
    #[account(mut, address = teacher_profile.authority)]
...
pub fn handler(ctx: Context<LessonApprove>, teacher_id: u32, lesson_id: u32) -> Result<()> {
    ...
    // Check current state
    if lesson.status_teacher != LessonState::Pending {
        return Err(errors::ErrorCode::LessonStateNotPending.into());
    }
